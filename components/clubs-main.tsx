...
//             <Tabs defaultValue="my-clubs" className="space-y-2">
...
//                                 <p className="text-sm font-serif text-secondary/50 font-medium">{club.members} people</p>
//                                 </div>
//                             </div>
//                             <div className="flex items-center gap-2">
...
//                                 <p className="text-sm font-serif text-secondary/50 font-medium">{club.nextMeeting}</p>
//                                 </div>
//                             </div>
//                         </div>
...
//                             </div>
//                             <div>
//                                 <p className="font-medium text-sm/4">{club.currentBook.title}</p>
...
//                                 <div>
//                                 <p className="text-sm/3 font-medium">Next Meeting</p>
//                                 <p className="text-sm font-serif text-secondary/50 font-medium">June 17, 2025</p>
//                                 </div>
//                             </div>
//                         </div>
...
//                         <Button className="bg-primary rounded-full hover:bg-primary-light text-primary-foreground">Join Club</Button>
//                     </CardFooter>
//                     </Card>
//                 ))}
//                 </TabsContent>
...
interface Club {
  ...
  name: string;
  description: string;
  // Assuming 'members' property for display, could be 'memberCount' from schema
  memberCount: number; 
  admin?: boolean; // True if the current user is an admin/owner of this club
  ownerId: string; // Owner's User ID (UUID)

  // These would typically come from more detailed club APIs or separate fetches
  currentBook?: {
    title: string;
    author: string;
    cover: string;
  ...
  // This is the current user's membership status for this specific club
  membershipStatus: 'ACTIVE' | 'PENDING' | 'REJECTED' | 'LEFT' | null; // Null if no membership
  pendingMemberships?: ClubMembershipRequest[]; // Populated only for admin clubs, fetched separately
}
...
export default function ClubsMain() {
  ...
  const [loadingClubs, setLoadingClubs] = useState(true); // For initial data fetch
    ...
    try {
      ...
      const clubsWithPending: Club[] = await Promise.all(data.map(async (club) => {
        if (club.admin) { // Check if the current user is an admin of this club
          try {
            const pendingRes = await fetch(`/api/clubs/${club.id}/pending-memberships`);
            if (!pendingRes.ok) {
              // Log error but don't stop the whole process if pending requests fail for one club
      ...
      setMyClubs(clubsWithPending);
    } catch (err: any) {
      toast.error(`Error fetching your clubs: ${err.message}`);
  ...
  const fetchDiscoverClubs = useCallback(async () => {
    try {
      const response = await fetch('/api/clubs/discover');
      if (!response.ok) {
        throw new Error("Failed to fetch discoverable clubs.");
      }
      const data: Club[] = await response.json();
      setDiscoverClubs(data);
    } catch (err: any) {
      toast.error(`Error fetching discoverable clubs: ${err.message}`);
  ...
  useEffect(() => {
    ...
    loadAllClubs();
  }, [fetchMyClubs, fetchDiscoverClubs]); // Depend on memoized fetch functions

  // --- API Integration Functions ( to refetch data) ---
 // --- NEW: Function to handle Create Club submission ---
  ...
  const handleJoinClub = async (clubId: string) => { // Changed clubId to string
    setLoadingAction(true);
    try {
